...
async function reset(page) {
  await page.evaluate(() => {
    if (typeof window.resetGlobe === 'function') window.resetGlobe();
    for (let i = 0; i < 4; i++) {
...
async function run(browserName, contextOpts, label) {
  ...
  // --- graph edge de-duplication: typing same box repeatedly does not duplicate activeEdgeProgress
  await reset(page);
  for (let k = 0; k < 20; k++) {
    await page.click('#otp-0');
    await page.keyboard.type('5');
  }
  const singleBoxEdgeCount = await page.evaluate(() => (window._debugGlobe?.() || activeEdgeProgress).length);
  record(
    P('repeated typing in single box creates 0 graph edges'),
    singleBoxEdgeCount === 0,
    `got activeEdgeProgress.length=${singleBoxEdgeCount}, want 0`
  );
  ...
  await page.keyboard.type('1234');
  await page.click('#otp-3');
  for (let k = 0; k < 10; k++) await page.keyboard.type('4');
  const multiBoxEdgeCount = await page.evaluate(() => (window._debugGlobe?.() || activeEdgeProgress).length);
  record(
    P('re-typing same digit in box 3 does not duplicate graph edges'),
    multiBoxEdgeCount === 6,
    `got activeEdgeProgress.length=${multiBoxEdgeCount}, want 6`
  );
